...
CAMERA_INDEX   = 0
CONF_THRESH    = 0.40          # giảm mạnh: nhận detection confidence thấp (chai trong suốt, góc nghiêng)
...
# ── Lọc box ──────────────────────────────────────────────
MAX_BOX_AREA_RATIO = 0.95      # nhận vật chiếm gần toàn frame
...
def filter_boxes(boxes_raw, frame_w, frame_h):
    """
    Lọc tối thiểu — chỉ loại box hoàn toàn vô lý:
    1. Diện tích = 0 hoặc âm
    2. Aspect ratio cực đoan (> 8.0)
    Tất cả kiểm tra ROI và area ratio đã được nới lỏng tối đa.
    """
    kept = []
    for b in boxes_raw:
        ...
        # Bỏ box kích thước âm hoặc bằng 0
        if bw <= 0 or bh <= 0:
            continue

        # Aspect ratio cực đoan
